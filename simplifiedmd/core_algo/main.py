...
for sentence in transformed_text:
	...
	tfisf_scores[sentence] = score
tfisf_scores = sorted(tfisf_scores.iteritems(), key=operator.itemgetter(1), reverse=True)
...
for i in range(len(transformed_text)):
    sent_feat_mat[i] = [theme_count(transformed_text[i], theme_words), 
                        ...
                        compute_tfisf(transformed_text[i], transformed_text), 
